...
>
  <FullscreenIcon sx={{ fontSize: 30 ,color:"white",backgroundColor:"rgba(0,0,0,0.5)", borderRadius:2,p:0.1}} /> {/* import from @mui/icons-material */}
</IconButton>
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog} fullScreen>
        <Box sx={{ position: "relative", height: "100%", bgcolor: "black" }}>
          ...
          <Box sx={{ ...thumbnailScrollStyles, position: "absolute", bottom: 10 }}>
            {images.map((img, index) => (
              <Box
                key={index}
      ...
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "center", width: "100%", mt: 3}}>
        ...
        <Box
          sx={{
            ...
            fontWeight: 600,
            backgroundColor: car.status === 'sold' ? '#C41E3A' : car.reserveMet ? '#32CD32' : null,
            fontFamily: "Inter",
            fontSize: { xs: 12, sm: 14 },
            width: "33.33%",
            textAlign: "center",
        ...
        <Box sx={{ flex: 1, height: 3, backgroundColor: car.status === 'sold' ? '#C41E3A' : car.reserveMet ? '#32CD32' : colors.buttoncolor }} />
      </Box>
