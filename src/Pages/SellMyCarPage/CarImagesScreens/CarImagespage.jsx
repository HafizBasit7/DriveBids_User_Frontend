...
const CarImages = () => {
  ...
  if(location.pathname.endsWith('/images') || location.pathname.endsWith('/images/')) {
    ...
    const allSectionsComplete = Object.values(validations).every(validation => validation.success);
      ...
      onClick={() => navigate("../")}>
        ...
        <Box width={{ xs: "95%", sm: "80%", md: "85%" }} mx="auto" mt={4}>
          {allSectionsComplete && (
            ...
            >
              <CheckCircleIcon sx={{ color: colors.buttoncolor, fontSize: 40 }} />
              <Box>
                <Typography sx={{ fontFamily: "Inter", fontWeight: 600, fontSize: 20, color: colors.buttoncolor }}>
                  All Images Uploaded Successfully!
                </Typography>
                <Typography sx={{ fontFamily: "Inter", fontSize: 16, color: "#666" }}>
            ...
            {steps.map((item, index) => (
              <Box
                ...
                sx={{
                  ...
                  backgroundColor: validations[item.status].success ? `${colors.buttoncolor}10` : "#f9f9f9",
                  borderRadius: 2,
                  cursor: "pointer",
                  transition: "all 0.3s ease",
                  "&:hover": { 
                    backgroundColor: validations[item.status].success ? `${colors.buttoncolor}15` : "#f0f0f0",
              ...
              >
                <Box display="flex" alignItems="center" gap={2}>
                  ...
                  <Box>
                    ...
                    <Typography
                      variant="body2"
                      color={validations[item.status].success ? colors.buttoncolor : "#6F6F6F"}
                      sx={{ fontFamily: "Inter" }}
                    >
