...
import {
  carDamageReportValidation,
  carDetailsValidation,
  carFeaturesValidation,
  carInspectionReportValidation,
...
const PostAds = () => {
  ...
  const carPricingCompletion = carPricingValidation.safeParse(
    carState.carPricing
  );
  const carInspectionReportCompletion = carInspectionReportValidation.safeParse(
    carState.carInspectionReport
  );
  const carDetailsCompletion = carDetailsValidation.safeParse(
    carState.carDetails
  );
  const imageCompletion = imagesValidation.safeParse(carState.images);
  const carDamageReportComplection = carDamageReportValidation.safeParse(
    carState.carDamageReport
  );
  const carFeaturesCompletion = carFeaturesValidation.safeParse(
    carState.features
  );
  ...
  const steps = [
    {
      title: "Car Details",
      status: carDetailsCompletion.success,
      steps: 14,
      icon: <ArticleIcon fontSize="large" />,
    },
    {
      title: "Car Features",
      status: carFeaturesCompletion.success,
      steps: 2,
    ...
    },
    {
      ...
      status: carInspectionReportCompletion.success,
      steps: 3,
      icon: <GppGoodIcon fontSize="large" />,
    },
    {
      title: "Damage Report",
      status: carDamageReportComplection.success,
      steps: 4,
      icon: <ReportIcon fontSize="large" />,
    },
    {
      title: "Car Pricing",
      status: carPricingCompletion.success,
      steps: 4,
  ...
  const handleNavigate = (index) => {
    if (
      index === 4 &&
      (carState?.carDamageReport === null ||
        carState?.carDamageReport?.damageReport?.length === 0)
    ) {
      dispatch({
        type: "SET_DRAFT",
        payload: { ...carState, carDamageReport: undefined },
      });
    }
